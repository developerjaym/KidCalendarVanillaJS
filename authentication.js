class AuthenticationService {
  ...
  async signIn(data) {
    return this.#fetchForToken(`${this.#environment.rootUrl}/auth/token`, data, true);
  }
  async #fetchForToken(url, data, basic = false) {
    try {
      const response = await fetch(url, this.#getPostRequestOptions(data, basic));
  ...
  isAuthenticated() {
    ...
    if(token) {
        return new Date(JSON.parse(atob(token.split('.')[1])).exp * 1000) > new Date();
    }
    return false;
  }
  #getToken() {
...
class AuthenticationModel extends Observable {
    ...
    constructor(authService) {
        ...
        this.#state = new AuthenticationState(this.#authenticationService.isAuthenticated());
    }
    start() {
...
class AuthenticationView extends Observer {
    ...
    onUpdate(state) {
        if (state.error) {
            new ErrorModal("That failed. Maybe try signing up or waiting or finding a new calendar app.").show();
        }
    }
    #showSignInForm() {
